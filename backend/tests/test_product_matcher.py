...
class TestProductMatcher:
    ...
    def test_match_growth_investor(self, sample_products):
        ...
        assert any(
            "growth" in m.name.lower() or "tech" in m.name.lower() for m in matches
        )
    ...
    def test_match_conservative_investor(self, sample_products):
        ...
        assert any(
            "conservative" in name or "bond" in name or "dividend" in name
    ...
    def test_get_top_n_products(self, sample_products):
        ...
        persona = Persona(
            interests=["Technology", "Growth"], risk_appetite="aggressive"
        )
...
def test_relevance_score_respects_persona_attributes():
    ...
    non_matching_score = matcher.calculate_relevance_score(
        non_matching_persona, product
    )
...
def test_learning_intent_prefers_masterclass_with_trigger_keywords_and_weight():
    products = [
        Product(
            id="et_masterclass",
            name="ET Masterclass & Careers",
            description="Expert-led courses and job market insights",
            target_audience=["Students", "Early Career"],
            categories=["Masterclass", "Careers"],
            core_benefit="Accelerate your career with practical learning",
            trigger_keywords=[
                "learn",
                "course",
                "upskill",
        ...
        Product(
            ...
            description="Live market data and trading tools",
            target_audience=["Active Traders"],
            categories=["Markets", "Trading"],
            core_benefit="Support active trading decisions",
            trigger_keywords=["stocks", "nifty", "ipo", "trading"],
            discovery_weight=5,
    ...
    persona = Persona(
        ...
        transition_intent="Looking to switch into AI roles",
    ...
    assert matches
    assert matches[0].id == "et_masterclass"
