    ...
    def calculate_relevance_score(self, persona: Persona, product: Product) -> float:
        ...
        persona_text = " ".join(
            [
                ...
                persona.career_stage or "",
                persona.transition_intent or "",
        ...
        if (persona.financial_goals or persona.learning_goals) and product.core_benefit:
            max_score += 0.20
            goals_text = " ".join(
                filter(None, [persona.financial_goals, persona.learning_goals])
            )
            goals_lower = goals_text.lower()
            benefit_lower = f"{product.core_benefit} {product.description}".lower()
        ...
        if persona.interests and product.categories:
            ...
            if interest_matches > 0:
                score += 0.20 * min(interest_matches / len(persona.interests), 1.0)

        # Check trigger keyword alignment with full persona text context
        if product.trigger_keywords and persona_text:
            max_score += 0.25
            keyword_matches = sum(
                1
                for keyword in product.trigger_keywords
                if keyword.lower() in persona_text
            )
            if keyword_matches > 0:
                score += 0.25 * min(
                    keyword_matches / len(product.trigger_keywords), 1.0
                )

        # Check risk appetite alignment
        if persona.risk_appetite:
            max_score += 0.15
            risk_lower = persona.risk_appetite.lower()
            if product.risk_profile:
                if risk_lower in [rp.lower() for rp in product.risk_profile]:
                    score += 0.15
        ...
        if max_score > 0:
            ...
            weight = max(1, min(product.discovery_weight, 10)) / 10
            return min(1.0, (normalized * 0.8) + (weight * 0.2))
