...
class CatalogLoader:
    ...
    def _resolve_catalog_path(catalog_path: str) -> Path:
        ...
        options = [candidate, backend_dir / candidate]

        # Support legacy values like "backend/data/et_catalog.json" when cwd is backend.
        if candidate.parts and candidate.parts[0] == "backend":
            options.append(backend_dir / Path(*candidate.parts[1:]))

        for option in options:
            if option.exists():
                return option

        # Fall back to backend-relative path for deterministic errors.
        return options[-1]
