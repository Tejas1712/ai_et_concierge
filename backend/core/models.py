...
class Persona(BaseModel):
    ...
    professional_background: Optional[str] = Field(
        None, description="User's professional background or occupation"
    )
    financial_goals: Optional[str] = Field(
        None, description="User's financial goals or objectives"
    )
    ...
    learning_goals: Optional[str] = Field(
        None,
        description="Learning goals such as upskilling, certifications, or domain mastery",
    )
    career_stage: Optional[str] = Field(
        None,
        description="Current career stage such as student, early-career, mid-career",
    )
    transition_intent: Optional[str] = Field(
        None,
        description="Whether the user is transitioning roles/domains and related intent",
    )
    interests: Optional[List[str]] = Field(
        default_factory=list, description="List of investment interests or preferences"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "learning_goals": "Transition into AI development",
                "career_stage": "Final-year engineering student",
                "transition_intent": "Moving from academics to AI engineering roles",
...
class Product(BaseModel):
    ...
    includes: List[str] = Field(
        default_factory=list, description="Included sections/services"
    )
    risk_profile: List[str] = Field(
        default_factory=list, description="Supported risk profiles"
    )
    trigger_keywords: List[str] = Field(
        default_factory=list, description="Keyword triggers for discovery"
    )
    discovery_weight: int = Field(
        default=1, description="Relative score for discovery ranking"
    )
